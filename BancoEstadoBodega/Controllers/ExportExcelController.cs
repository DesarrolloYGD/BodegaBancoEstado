...
namespace BancoEstadoBodega.Controllers
{
    public class ExportExcelController : Controller
    {
        ...
        public ActionResult ExportDataCritico()
        {
            ...
            string query = "select codigo, Nombre, Cliente.Alias, cantidadTotal from cliente inner join Producto on Producto.IDClienteFK = Cliente.IDCliente " + condicion + "(cantidadtotal*100)/(stock_ideal) <= 20";
            DataTable dt = new DataTable();
            dt.TableName = "PRODUCTO";
            con.Open();
            SqlDataAdapter da = new SqlDataAdapter(query, con);
